        ...
        *buffer = '\0';
        ...
        if (do_gases && (planet->gases > 0)) {
            ...
            for (i = 0; i < planet->gases; i++) {
                ...
                if (planet->atmosphere[i].inspired_partial_pressure(planet->surf_pressure) > StarGen::Gases::gases[index].max_ipp) {
                    poisonous = true;
                }

                if (((planet->atmosphere[i].surf_pressure / planet->surf_pressure) > .0005) || poisonous) {
        ...
        fprintf(file,
            "'%s %d', %5.3Lf, %5.3Lf, %5.3Lf, %5.3Lf, %d, "
                ...
                planet->cloud_cover,
                planet->ice_cover,
                (do_gases && (planet->gases > 0)) ? buffer : "",
            ...
            *buffer = '\0';
            ...
            if (do_gases && (moon->gases > 0)) {
                ...
                for (i = 0; i < moon->gases; i++) {
                    ...
                    if (moon->atmosphere[i].inspired_partial_pressure(moon->surf_pressure) > StarGen::Gases::gases[index].max_ipp) {
                        poisonous = true;
                    }

                    if (((moon->atmosphere[i].surf_pressure / moon->surf_pressure) > .0005) || poisonous) {
                        sprintf(ptr, "%s %.1Lf%% %.0Lfmb (ipp:%.0Lf)%s; ",
                                        StarGen::Gases::gases[index].symbol,
...
const char *texture_name(planet_type type) {
    switch (type) {
        ...
        case tRock:
            return "callisto.jpg";
        case tVenusian:
            return "venuslike.jpg";
        case tTerrestrial:
            return "Earthlike.png";
        case tSubSubGasGiant:
            return "x.jpg";
        case tSubGasGiant:
            return "gasgiant.jpg";
        case tGasGiant:
            return "jupiterlike.jpg";
        case tMartian:
            return "mars.jpg";
        case tWater:
            return "x.jpg";
...
void create_svg_file(FILE *file_arg, planet_pointer innermost_planet, const char *path, const char *file_name, const char *svg_ext, const char *prognam) {
...

        {
            ...
            for (x = floor; x <= ceiling; x += 1.0) {
                ...
                for (n = 1.; n < 9.9; n++) {
                    ...
                    if ((max_log == 0.0) && (outer_edge < pow(10., x + log10(n)))) {
                        max_log = x + log10(n);
                    }
                }
            }
...
void print_description(FILE* file, const char* opening, planet_pointer planet, const char* closing) {
    ...
    long double atmosphere = (planet->surf_pressure / EARTH_SURF_PRES_IN_MILLIBARS);
    ...
    if (atmosphere < 0.001) {
        ...
        if (planet->type != tWater) {
            if (seas < 25.) {
                LPRINT ("Arid") // 25% surface is water
            } else if (seas < 50.) {
                LPRINT ("Dry")
            } else if (seas > 80.) {
                LPRINT ("Wet")
        ...
        if (clouds < 10.) {
            LPRINT ("Cloudless") // 10% cloud cover
        } else if (clouds < 40.) {
            LPRINT ("Few clouds")
        } else if (clouds > 80.) {
            LPRINT ("Cloudy")
        ...
        if (planet->surf_pressure < MIN_O2_IPP) {
            ...
            LPRINT ("Thin atmosphere")
        } else if (atmosphere > MAX_HABITABLE_PRESSURE / EARTH_SURF_PRES_IN_MILLIBARS) { // Dole, pp. 18-19
            LPRINT ("Unbreathably thick atmosphere")
        } else if (atmosphere > 2.0) {
            LPRINT ("Thick atmosphere")
    ...
    if (planet->gases > 0) {
        ...
        for (i = 0; i < planet->gases; i++) {
            ...
            if ((planet->atmosphere[i].surf_pressure / planet->surf_pressure) > .01) {
...
//
// This function lists the gases whose atomic/molecular weight is large enough that it is retained.
//

#define MOL_PRINTF(x,y) { if (y >= weight) { \
                         {if (++count > max) {fprintf(file, "..."); return;} \
                          else LPRINT(x)} } }
void list_molecules(FILE* file, long double weight) {
    int count = 0;
    int max = 8;
    int first = true;
...
void html_thumbnails(planet_pointer innermost_planet, FILE* file, char* system_name, char* url_path, char* system_url,
                     char* svg_url, char* file_name, bool details, bool terrestrials, bool int_link, bool do_moons,
                     bool output_svg) {
    planet_pointer planet;
    StarGen::Sun* sun = innermost_planet->sun;
    int counter;
    ...
    if (file_name[0] == '\0') {
        ...
        fprintf(file, "<a href='%s'>%s</a>", system_url, system_name);
    }
    ...
    {
        fprintf(file, "<table border=0 cellspacing=0 cellpadding=3 bgcolor='" BGSPACE "' width='100%%'>\n");
        ...
        for (planet = innermost_planet, counter = 1; planet != NULL; planet = planet->next_planet, counter++) {
            ...
            if (planet->type == tAsteroids) {
                ppixels = (int)(25.0 + ( 5.0 * log ((planet->mass * SUN_MASS_IN_EARTH_MASSES) / ASTEROID_MASS_LIMIT)));
            }
            ...
            if ((planet->type == tGasGiant) || (planet->type == tSubSubGasGiant) || (planet->type == tSubGasGiant)) {
                sprintf(&info[0], "%s: %.1LfEM (c. %.0Lf&deg;)", ptype, planet->mass * SUN_MASS_IN_EARTH_MASSES, planet->estimated_temp);
            ...
            for (moon = planet->first_moon, moons=1; do_moons && moon != NULL; moon = moon->next_planet, moons++) {
                // DISPLAY THEM
                const char *mtype = type_string (moon->type);
                int mpixels = ((int)( sqrt(moon->radius / KM_EARTH_RADIUS) * 30.)) + 1;

                if ((moon->type == tGasGiant) || (moon->type == tSubSubGasGiant) || (moon->type == tSubGasGiant)) {
...
void html_decribe_planet(planet_pointer planet, int counter, int moons, bool do_gases, char* url_path, FILE* file) {
    char planet_id[80];
    ...
    fprintf(file, "%5.3Lf Earth masses<br>", planet->mass * SUN_MASS_IN_EARTH_MASSES);
    ...
    if ((planet->type != tGasGiant) && (planet->type != tSubGasGiant) && (planet->type != tSubSubGasGiant)) {
        ...
        if (planet->greenhs_rise > 0.1) {
            fprintf(file, "<br>%+4.1Lf&deg; C greenhouse effect", planet->greenhs_rise);
...

        {
            ...
            if (fabs(planet->high_temp - planet->max_temp) > 10 || fabs(planet->low_temp - planet->min_temp) > 10) {
            ...
            fprintf(file, "</table></center></td></tr>\n");
        }
    }
    ...
    if (planet->orb_period > DAYS_IN_A_YEAR) {
        fprintf(file, "<br>%4.2Lf Earth years", planet->orb_period / DAYS_IN_A_YEAR);
    }
...
void html_describe_system(planet_pointer innermost_planet, bool do_gases, char* url_path, FILE* file) {
    planet_pointer planet;
    int counter;
    planet_pointer moon;
    int moons;
