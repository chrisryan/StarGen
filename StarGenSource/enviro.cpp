...
long double day_length(planet_pointer planet) {
    ...
    bool giant = (planet->type == tGasGiant || planet->type == tSubGasGiant || planet->type == tSubSubGasGiant);
    long double k2;
...
int inclination(long double orb_radius) {
    int temp = (int)(pow(orb_radius,0.2) * StarGen::Random::about(EARTH_AXIAL_TILT,0.4));
    return (temp % 360);
}
...
long double cloud_fraction(long double surf_temp, long double smallest_MW_retained,
                           long double equat_radius, long double hydro_fraction) {
...
long double planet_albedo(long double water_fraction, long double cloud_fraction,
                            long double ice_fraction, long double surf_pressure) {
...
//
long double gas_life(long double molecular_weight, planet_pointer planet) {
...
void calculate_surface_temp(planet_pointer planet, bool first,
    ...
    if (first) {
        ...
        greenhouse_temp = green_rise(opacity(planet->molec_weight, planet->surf_pressure),
                                     effective_temp,
                                     planet->surf_pressure);
    ...
    if (planet->greenhouse_effect && planet->max_temp < planet->boil_point) {
        if (StarGen::Stargen::isVerbose(0x0010)) {
            fprintf (stderr, "Deluge: %s %d max (%Lf) < boil (%Lf)\n",
                    planet->sun->name, planet->planet_no,
    ...
    if ((planet->high_temp >= planet->boil_point) && (!first)
        && !((int)planet->day == (int)(planet->orb_period * 24.0) || (planet->resonant_period))) {
    ...
    if (StarGen::Stargen::isVerbose(0x0020)) {
        fprintf (stderr, "%5.1Lf AU: %5.1Lf = %5.1Lf ef + %5.1Lf gh%c "
                ...
                planet->ice_cover, planet->albedo);
    }
}
