...
void coalesce_planetesimals(long double a, long double e, long double mass, long double crit_mass,
    ...
    for (the_planet = planet_head; the_planet != NULL; the_planet = the_planet->next_planet) {
        ...
        if (diff > 0.0) {
            ...
            dist2 = the_planet->a - (the_planet->a * (1.0 - the_planet->e) * (1.0 - reduced_mass));
        } else {
            dist1 = a - (a * (1.0 - e) * (1.0 - reduced_mass));
            // x perihelion
            reduced_mass = pow((the_planet->mass / (1.0 + the_planet->mass)), (1.0 / 4.0));
            dist2 = (the_planet->a * (1.0 + the_planet->e) * (1.0 + reduced_mass)) - the_planet->a;
        }

        if ((fabs(diff) <= fabs(dist1)) || (fabs(diff) <= fabs(dist2))) {
            long double new_dust = 0;
            long double new_gas = 0;
            long double new_a = (the_planet->mass + mass) / ((the_planet->mass / the_planet->a) + (mass / a));
...
planet_pointer dist_planetary_masses(long double stell_mass_ratio, long double stell_luminosity_ratio,
                                     long double inner_dust, long double outer_dust, long double outer_planet_limit,
                                     long double dust_density_coeff, planet_pointer seed_system, bool do_moons) {
    long double a;
    long double e;
    long double mass;
